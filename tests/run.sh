...
PATH2LIB="../PassPlugin.so" # Specify your build directory in the project

# ACTION NEEDED: Choose the correct pass when running.
PASS=bit-theft # Choose either -fplicm-correctness ...
...
rm -f default.profraw *_prof *_bit_theft *.bc *.profdata *_output *.ll
...
opt -load-pass-plugin="${PATH2LIB}" -passes="${PASS}" ${1}.profdata.bc -o ${1}.bit_theft.bc >/dev/null

# Generate binary excutable before FPLICM: Unoptimzed code
clang ${1}.ls.bc -o ${1}_no_bit_theft
# Generate binary executable after FPLICM: Optimized code
clang ${1}.bit_theft.bc -o ${1}_bit_theft

# Produce output from binary to check correctness
./${1}_bit_theft >bit_theft_output

echo -e "\n=== Program Correctness Validation ==="
if [ "$(diff correct_output bit_theft_output)" != "" ]; then
	echo -e ">> Outputs do not match\n"
else
	echo -e ">> Outputs match\n"
	# Measure performance
	echo -e "1. Performance of unoptimized code"
	time ./${1}_no_bit_theft >/dev/null
	echo -e "\n\n"
	echo -e "2. Performance of optimized code"
	time ./${1}_bit_theft >/dev/null
	echo -e "\n\n"
fi
