...
int getMap(int *map, int x, int y, int N) {
    return map[x * N + y];
}
...
bool find(int *map, bool *visited, int x, int y, bool isArrivalLayer, int N, int depth) {
  if (x < 0 || x >= N || y < 0 || y >= N || getVisited(visited, x, y, isArrivalLayer, N) || getMap(map, x, y, N) == 1) {
    return false;
  }
